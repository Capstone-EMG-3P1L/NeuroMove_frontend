  ...
  const handleNext = async () => {
    ...
    try {
      ...
      if (calibrationSessionId) {
        if (onboardingId) {
          await onboardingApi.updateCalibrationStep({ onboardingId, calibrationSessionId, step: "REST" }).catch(() => {});
        } else {
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-6">
      ...
      >
        <div className="flex flex-col items-center justify-center p-10 pb-6 gap-6">
          <div className="flex items-center gap-2 mb-1">
            {[1, 2, 3, 4, 5].map((step) => (
              <div
                key={step}
                className={`w-2 h-2 rounded-full ${step === 2 ? "bg-primary" : step < 2 ? "bg-primary/40" : "bg-border"}`}
              />
            ))}
          </div>
        ...
        <div className="px-6 pb-6">
          ...
          >
            {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : "정면을 바라봐주세요"}
          </Button>
        </div>
      </motion.div>
    </div>
